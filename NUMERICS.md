...
| mLiftOver to/from a genome | `mliftover --to hg38` / `--platform hg38 --to <array>` | none -- R's `mLiftOver` is array-only | No oracle, so the gate is an independent reference built in-test from the same two store files with yame + awk (coordinate join, replicate mean): row count equals the genome's CpG universe, covered rows identical, M within 1 of the reference (whose betas pass through 3-decimal text), and the lift back returns every non-replicate covered beta within 0.0051. Cross-checked once against methscope: `classify` on the C-lifted MSA file reproduces the hand-built prototype's sex calls and confidences to 3 decimals (10/10). |
